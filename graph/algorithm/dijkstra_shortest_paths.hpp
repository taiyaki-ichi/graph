...
namespace graph
{
		...
		std::unordered_map<unsigned int, std::optional<T>>* distance)
	{
		...
		auto get_not_checked_vertex = [&vertex, &checked]()->std::vector<unsigned int>
		{
		...
		auto get_min_index_and_distance = [distance](std::vector<unsigned int>&& list)
			->unsigned int
		{
		...
		//すべての辺をチェックしたかどうか
		auto all_vertex_is_checked = [&checked]()->bool {
			bool result = true;
			for (auto v : checked)
				if (!v.second) {
					result = false;
